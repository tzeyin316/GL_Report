...
# ---------- CONFIG ----------
final_header = ['Date','Reference Code','Account Code','Account Name','Description 1','Desciption 2','Tax','Debit (RM)','Credit (RM)','Ending Balance (RM)']

combined_df = pd.DataFrame()  # empty DataFrame to collect all sheets
...
if uploaded_file is not None:
    ...
    clean_df = pd.DataFrame()  # empty DataFrame to collect all sheets
    ...
    for each_sheet in file.sheet_names:
        ...
        for row in values:
            ...
            if isinstance(row[0], str) and row[0].strip().startswith("Code :"):
                subAccCode, subAccName = row[0].replace("Code :", "").strip().split(" ", 1)
                ...
                final_data.append([date, ref, subAccCode, subAccName, desc1, desc2, tax, debit, credit, balance])
